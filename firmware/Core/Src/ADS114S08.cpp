...
int init() {
    ...
    datarate.u.DR = (uint8_t)DatarateSelectionDrEnum::SEL_200_SPS;
    ...
    inpmux.u.MUXN = (uint8_t)INPMUXEnum::AINCOM;
    reg_write(INPMUX_ADDR, inpmux.Value);

    // Set appropriate PGA gain and if in bypass mode by write to register
    // PGA(ADR = 03h).
    pga.u.GAIN = (uint8_t)PGAPageSelectionEnum::GAIN_1;
    pga.u.PGA_EN = (uint8_t)PGAEnableEnum::ENABLED;
    reg_write(PGA_ADDR, pga.Value);

    sys.u.CRC_ = (uint8_t)SysControlCrcEnum::ENABLE;
    sys.u.SENDSTAT = (uint8_t)SysControlStatusEnum::ENABLE;
    sys.u.SYS_MON = (uint8_t)SysControlSysmonEnum::NORMAL;
...
HAL_StatusTypeDef start_by_command() {
    uint8_t tx[1];
    tx[0] = START_CMD;
    return HAL_SPI_Transmit(&hspi, tx, sizeof(tx));
}
...
static HAL_StatusTypeDef reg_read(uint8_t reg, uint8_t* data) {
    ...
    HAL_StatusTypeDef status =
        HAL_SPI_TransmitReceive(&hspi, txrx, txrx, sizeof(txrx), HAL_MAX_DELAY);
    if(status == HAL_OK) {
        *data = txrx[2];
    }
    return status;
